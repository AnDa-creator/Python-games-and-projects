...
# exits_to = tuple([{x: locations[x]} for x in locations.keys() for item in exits[x].values() if item == 5])
# print(exits_to)
exit_final = []

exits_to = tuple([{x: locations[x]} for location in locations for x in locations.keys() for item in exits[x].values() if item == location])
